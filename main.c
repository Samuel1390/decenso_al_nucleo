...
void equip_armor(Player* player, Armor* armor) {
    player->base_char.defense = armor;
    printf("%s se ha equipado con %s\n", player->base_char.name, armor->base_item.name);
    enter_to_continue();
}
...
int main(void) {
    ...
    ObjectData iron_sword, iron_armor;
    iron_sword.type = TYPE_WEAPON;
    iron_sword.data.weapon = base_iron_sword;
    ...
    Player player;
    init_character(&player.base_char, "Caballero", 80, 100, 100, &iron_armor, 1, 100, 0, &iron_sword);
    ...
    init_character(&enemy1, "Goblin", 10, 80, 80, &iron_armor, 1, 100, 0, NULL);
    init_character(&enemy2, "Goblin", 20, 90, 90, &iron_armor, 2, 110, 0, NULL);
